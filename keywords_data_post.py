...
def range_limited_int_type(arg):
    ...
    if f < 1 or f > 700:
        raise argparse.ArgumentTypeError(
            "Argument must be < " + str(1) + " and > " + str(700)
        )
    return f
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--input", required=True, type=str, help="List of keywords to request."
    )
    parser.add_argument(
        "--output",
        default="keywords-data-tasks",
        type=str,
        help='Output basename (default: "keywords-data-tasks").',
    )
    parser.add_argument(
        "--language_code",
        default="fr",
        type=str,
        help='Language code for requests (default: "fr")',
    )
    parser.add_argument(
        "--location_code",
        default=2250,
        type=int,
        help='Location code for requests (default: "2250" for France, get other codes on <https://api.dataforseo.com/v3/serp/google/locations>)',
    )
    parser.add_argument(
        "--batch",
        default=700,
        type=range_limited_int_type,
        help="Max number of tasks per batch. Max 700. Each batch costs 0.05$ (default: 700).",
    )
    parser.add_argument(
        "--delay",
        default=1,
        type=float,
        help="Delay in seconds between batches of requests (default: 1).",
    )
    parser.add_argument(
        "--sep", default=";", type=str, help='CSV file separator (default: ";").'
    )
                ...
                for task in response["tasks"]:
                    ...
                    with open(filename, "a", newline="") as file:
                        writer = csv.DictWriter(
                            file, fieldnames=fields, delimiter=args.sep
                        )
